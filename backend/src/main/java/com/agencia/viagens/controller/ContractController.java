        ...
        public class ContractController {
            ...
            @Operation(
                    summary = "Anotar pagamentos do cliente",
                    description = """
                            Lógica para pagamentos de contratos parcelados."""
            )
            @ApiResponses(value = {
                    @ApiResponse(responseCode = "200", description = "Pagamento efetuado"),
                    @ApiResponse(responseCode = "404", description = "Falha ao marcar pagamento")
            })
            @PostMapping("/{id}/payment")
            public ResponseEntity<ContractResponseDTO> addPayment(
                    @PathVariable UUID id,
